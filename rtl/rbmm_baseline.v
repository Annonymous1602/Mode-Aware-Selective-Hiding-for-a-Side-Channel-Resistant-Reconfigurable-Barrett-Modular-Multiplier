            ...
            case (state)
                ...
                S_IDLE: begin
                    ...
                    if (start) begin
                        ...
                        if (mode == 2'b00) begin
                            // 9 independent 64-bit BMM lanes.
                            // Each operand is zero-extended to 128 bits.
...
module rbmm_bank_9core (
    ...
    // 9 physical 128-bit multiplier cores
    mult #(128, 4, 32) u0 (.A({1'b0, ca0}), .B({1'b0, cb0}), .P(cp0));
    mult #(128, 4, 32) u1 (.A({1'b0, ca1}), .B({1'b0, cb1}), .P(cp1));
    mult #(128, 4, 32) u2 (.A({1'b0, ca2}), .B({1'b0, cb2}), .P(cp2));
    mult #(128, 4, 32) u3 (.A({1'b0, ca3}), .B({1'b0, cb3}), .P(cp3));
    mult #(128, 4, 32) u4 (.A({1'b0, ca4}), .B({1'b0, cb4}), .P(cp4));
    mult #(128, 4, 32) u5 (.A({1'b0, ca5}), .B({1'b0, cb5}), .P(cp5));
    mult #(128, 4, 32) u6 (.A({1'b0, ca6}), .B({1'b0, cb6}), .P(cp6));
    mult #(128, 4, 32) u7 (.A({1'b0, ca7}), .B({1'b0, cb7}), .P(cp7));
    mult #(128, 4, 32) u8 (.A({1'b0, ca8}), .B({1'b0, cb8}), .P(cp8));

    assign p64[0*256 +: 256] = cp0;
    assign p64[1*256 +: 256] = cp1;
    assign p64[2*256 +: 256] = cp2;
    assign p64[3*256 +: 256] = cp3;
    assign p64[4*256 +: 256] = cp4;
    assign p64[5*256 +: 256] = cp5;
    assign p64[6*256 +: 256] = cp6;
    assign p64[7*256 +: 256] = cp7;
    assign p64[8*256 +: 256] = cp8;
